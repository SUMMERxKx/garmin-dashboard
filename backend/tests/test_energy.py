...
def test_mifflin_st_jeor_matches_hand_calculation(profile: models.Profile) -> None:
    ...
    assert result.formula is models.BmrFormula.MIFFLIN_ST_JEOR


def test_mifflin_female_offset() -> None:
    p = models.Profile(user_id="x", sex=models.Sex.FEMALE, birth_date=date(2003, 5, 1), height_cm=180.0)
    # same inputs, -161 instead of +5 -> 166 kcal lower
    assert energy.bmr(p, 80.0, date(2026, 9, 3)).kcal == pytest.approx(1649.0)


def test_katch_mcardle_used_when_lean_mass_known(profile: models.Profile) -> None:
    ...
    assert result.formula is models.BmrFormula.KATCH_MCARDLE


def test_the_two_formulas_agree_within_a_few_percent(profile: models.Profile) -> None:
...
def test_formula_choice_is_always_explained(profile: models.Profile) -> None:
    """A step change in the target must never be mysterious."""
    assert energy.bmr(profile, 80.0, date(2026, 9, 3)).reasons[0].code is reasons.ReasonCode.BMR_FORMULA_MIFFLIN_ST_JEOR
    assert energy.bmr(profile, 80.0, date(2026, 9, 3), lean_mass_kg=64.8).reasons[0].code is reasons.ReasonCode.BMR_FORMULA_KATCH_MCARDLE


def test_bmr_tracks_weight_not_a_stale_age(profile: models.Profile) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (2760.0, 2250.0, models.BalanceState.DEFICIT),
        (2400.0, 2650.0, models.BalanceState.SURPLUS),
        (2400.0, 2420.0, models.BalanceState.MAINTENANCE),
    ],
)
def test_balance_states(burned: float, consumed: float, state: models.BalanceState) -> None:
...
def test_rest_day_reads_as_maintenance_on_a_fixed_target() -> None:
    ...
    assert energy.energy_balance(2400.0, 2350.0).state is models.BalanceState.MAINTENANCE


def test_resistance_training_caveat_is_surfaced() -> None:
    lifting = models.Activity(provider_id="1", kind=models.ActivityKind.RESISTANCE, type_raw="strength_training", duration_min=58.0)
    codes = [r.code for r in energy.energy_balance(2760.0, 2250.0, activities=[lifting]).reasons]
    assert reasons.ReasonCode.RESISTANCE_CALORIES_UNRELIABLE in codes


def test_no_caveat_for_a_run() -> None:
    run = models.Activity(provider_id="2", kind=models.ActivityKind.RUNNING, type_raw="running", duration_min=52.0)
    codes = [r.code for r in energy.energy_balance(3100.0, 2350.0, activities=[run]).reasons]
    assert reasons.ReasonCode.RESISTANCE_CALORIES_UNRELIABLE not in codes
