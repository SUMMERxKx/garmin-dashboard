...
def test_the_real_target_is_internally_consistent(target: models.MacroTarget) -> None:
    assert abs(target.implied_kcal - target.kcal) < 10.0


def test_resolve_entry_scales_by_servings(chicken: models.Food) -> None:
...
def test_day_totals_sums_snapshots(chicken: models.Food) -> None:
    entries = [conftest.make_entry(date(2026, 9, 3), chicken, 2.0), conftest.make_entry(date(2026, 9, 3), chicken, 1.5)]
...
def test_editing_a_food_cannot_rewrite_history(chicken: models.Food) -> None:
    """macros_snapshot is denormalised on write, so last month's dashboard stays true."""
    entry = conftest.make_entry(date(2026, 8, 1), chicken, 2.0)
...
def test_target_on_uses_the_target_in_force_that_day() -> None:
    august = models.MacroTarget(effective_from=date(2026, 8, 1), goal=models.GoalType.CUTTING,
                         kcal=2400.0, protein_g=180.0, carbs_g=260.0, fat_g=70.0)
    september = models.MacroTarget(effective_from=date(2026, 9, 3), goal=models.GoalType.CUTTING,
...
def test_remaining_can_go_negative(target: models.MacroTarget) -> None:
    """Going over is information, not an error."""
    over = models.MacroTotals(kcal=2500.0, protein_g=200.0, carbs_g=280.0, fat_g=70.0)
    assert nutrition.remaining(over, target).kcal == pytest.approx(-150.0)


def test_remaining_plus_consumed_equals_target(target: models.MacroTarget) -> None:
    totals = models.MacroTotals(kcal=1930.0, protein_g=151.0, carbs_g=214.0, fat_g=61.0)
...
def test_adherence_flags_protein_shortfall(target: models.MacroTarget) -> None:
    totals = models.MacroTotals(kcal=2140.0, protein_g=151.0, carbs_g=230.0, fat_g=68.0)
    ...
    assert reasons.ReasonCode.PROTEIN_UNDER_TARGET in codes
    assert reasons.ReasonCode.CALORIES_UNDER_TARGET in codes


def test_protein_met_within_tolerance(target: models.MacroTarget) -> None:
    totals = models.MacroTotals(kcal=2350.0, protein_g=176.0, carbs_g=260.0, fat_g=65.0)
    assert nutrition.adherence(totals, target).protein_target_met is True


def test_no_food_logged_is_its_own_reason(target: models.MacroTarget) -> None:
    result = nutrition.adherence(models.MacroTotals(), target, entry_count=0)
    assert next(r.code for r in result.reasons) is reasons.ReasonCode.NO_FOOD_LOGGED


def test_validate_food_accepts_a_real_label(chicken: models.Food) -> None:
    assert nutrition.validate_food(chicken) is True


def test_validate_food_catches_a_misplaced_decimal(chicken: models.Food) -> None:
    """The guard exists to catch 10x errors from a vision misread or a typo."""
    chicken.kcal = 1650.0
    assert nutrition.validate_food(chicken) is False


def test_serving_basis_travels_with_the_entry(chicken: models.Food) -> None:
    """The engine never converts raw<->cooked; the basis must survive to the UI."""
    entry = conftest.make_entry(date(2026, 9, 3), chicken, 2.0)
    assert entry.serving_basis is models.ServingBasis.RAW


def test_adherence_streak_uses_each_days_own_target() -> None:
    targets = [models.MacroTarget(effective_from=date(2026, 9, 1), goal=models.GoalType.CUTTING,
                           kcal=2350.0, protein_g=180.0, carbs_g=260.0, fat_g=65.0)]
    days = [
        (date(2026, 9, 1), models.MacroTotals(protein_g=185.0)),
        (date(2026, 9, 2), models.MacroTotals(protein_g=178.0)),  # within 5 g tolerance
        (date(2026, 9, 3), models.MacroTotals(protein_g=181.0)),
    ]
    assert nutrition.adherence_streak(days, targets) == 3

    days[1] = (date(2026, 9, 2), models.MacroTotals(protein_g=150.0))
