...
def declining(days: int = 28, start: float = 80.5, per_day: float = -0.06,
              noise: float = 0.0, end: date = TODAY) -> list[tuple[date, float]]:
    out = []
    for i in range(days):
...
def test_ema_smooths_a_single_spike() -> None:
    ...
    data = [(TODAY - timedelta(days=i), 80.0) for i in range(9, -1, -1)]
    data[-1] = (TODAY, 83.0)  # one bad morning
    value = weight.ema_on(data, TODAY)
...
def test_ema_is_time_weighted_so_gaps_count_more() -> None:
    ...
    tight = [(TODAY - timedelta(days=1), 80.0), (TODAY, 78.0)]
    loose = [(TODAY - timedelta(days=10), 80.0), (TODAY, 78.0)]
    tight_value = weight.ema_on(tight, TODAY)
    loose_value = weight.ema_on(loose, TODAY)
...
def test_trend_recovers_a_known_slope() -> None:
    ...
    result = weight.trend(data, TODAY)
...
def test_trend_needs_three_points() -> None:
    assert weight.trend([(TODAY, 80.0), (TODAY - timedelta(days=1), 80.1)], TODAY) is None


def test_rate_of_change_is_kg_per_week() -> None:
    rate = weight.rate_of_change(declining(days=14, per_day=-0.07), TODAY)
...
def test_change_over_uses_smoothed_values() -> None:
    ...
    change = weight.change_over(declining(days=30, per_day=-0.05, noise=0.6), TODAY, 21)
    assert change is not None
    assert -1.6 < change < -0.6


def test_change_over_without_history_is_none() -> None:
    assert weight.change_over([(TODAY, 80.0)], TODAY, 30) is None


def test_flat_and_quiet_is_a_real_plateau() -> None:
    data = [(TODAY - timedelta(days=i), 79.0 + (0.05 if i % 2 else -0.05)) for i in range(21)]
    result = weight.plateau(data, TODAY)
    assert result is not None
    assert result.is_plateau is True
    assert result.reasons[0].code is ReasonCode.PLATEAU_DETECTED
...
def test_flat_but_noisy_declines_to_call_a_plateau() -> None:
    ...
    data = [(TODAY - timedelta(days=i), 79.0 + (1.4 if i % 2 else -1.4)) for i in range(21)]
    result = weight.plateau(data, TODAY)
    assert result is not None
    assert result.is_plateau is False
    assert result.reasons[0].code is ReasonCode.LIKELY_WATER_FLUCTUATION


def test_a_clear_decline_is_not_a_plateau() -> None:
    result = weight.plateau(declining(days=21, per_day=-0.06), TODAY)
    assert result is not None
    assert result.is_plateau is False
    assert result.reasons[0].code is ReasonCode.WEIGHT_TREND_DOWN
